...
public class Nfs3Test {
  ...
  public static void setUp() throws Exception {
    ...
    store = new NFSv3FileSystemStore(new URI("nfs://localhost:" + nfsServerPort + "/"), conf);
  ...
  public void NullConfiguration() throws Exception {
    ...
    // Null configuration
    {
      NFSv3FileSystemStore temp = new NFSv3FileSystemStore(new URI("nfs://localhost:" + nfsServerPort + "/"), null);
      try {
    ...
    
    {
      ...
      conf.setBoolean("mambo.test", true);
      conf.set("mambo.test.mountpath", MOUNT_PATH);
      NFSv3FileSystemStore temp = new NFSv3FileSystemStore(new URI("nfs://localhost:" + nfsServerPort + "/"), conf);
    ...
    
    {
      ...
      try {
        temp = new NFSv3FileSystemStore(new URI("nfs://localhost:" + nfsServerPort + "/"), conf);
        temp.initialize(new URI("nfs://localhost:" + nfsServerPort + "/"), conf);
        temp.shutdown();
      } catch(IOException exception) {
        errored = true;
