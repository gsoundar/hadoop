...
public class NFSv3FileSystemStore extends RpcClient {
  ...
  public NFSv3FileSystemStore(URI uri, Configuration conf) throws IOException {
    ...
    if (conf != null) {
      ...
      if (credentialsFlavor != null) {
        ...
        else if (credentialsFlavor.equalsIgnoreCase("AUTH_SYS")
            || credentialsFlavor.equalsIgnoreCase("AUTH_UNIX")) {
  ...
  public void initialize(URI uri, Configuration conf) throws IOException {
    ...
    if (conf.getBoolean("mambo.test", false) == false) {
      ...
      int mountdPort =
          portmapClient.getport(MountClient.MOUNTD_PROGRAM, MountClient.MOUNTD_VERSION,
              PortmapMapping.TRANSPORT_TCP);
      MountClient mountClient = new MountClient(uri.getHost(), mountdPort, conf);
      ...
      if (mountMNTResponse.getStatus() != Nfs3Status.NFS3_OK) {
        LOG.error("failed to mount root path " + mountDir + " from host " + uri.getHost()
            + ", status=" + mountMNTResponse.getStatus());
        return;
      }
