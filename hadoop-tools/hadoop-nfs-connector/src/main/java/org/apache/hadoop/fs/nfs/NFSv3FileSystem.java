...
public class NFSv3FileSystem extends FileSystem {
  ...
  public void initialize(URI uri, Configuration conf) throws IOException {
    ...
    int numEndpoints = conf.getInt("fs.nfs.extraendpoints", 0);
    LOG.info("Got " + numEndpoints + " endpoints from config");
    for(int i = 1; i < numEndpoints; ++i) {
      try {
        endpoints.add(new URI(conf.get("fs.nfs.endpoint." + i, null)));
      } catch(Exception e) {
        LOG.error("Got a bad entry for extra endpoint", e);
      }
    }
    endpoints.add(uri);
    
    // Make sure to check connection to all the endpoints
    for(URI u : endpoints) {
      scheme = u.getScheme();
      ...
      int port = getNFSPortFromPortMap(u);
      if (port <= 0) {
        throw new IOException("NFS program is not registered with portmap!");
      }
  
      // Verify the port from the URI.
      int portInUri = u.getPort();
      if (portInUri != -1 && portInUri != port) {
        LOG.warn("Port specified in URI is different from port gotten from portmap: " + u.getPort()
            + "!=" + port);
      }
    }
    this.uri = uri;
    ...
    readBlockSizeBits =
        conf.getInt("fs." + uri.getScheme() + ".readblockbits", DEFAULT_READ_BLOCK_SIZE_BITS);
    writeBlockSizeBits =
        conf.getInt("fs." + uri.getScheme() + ".writeblockbits", DEFAULT_WRITE_BLOCK_SIZE_BITS);
    splitSizeBits =
        conf.getInt("fs." + uri.getScheme() + ".splitsizebits", DEFAULT_NFS_SPLIT_SIZE_BITS);
    ...
    LOG.info("Using credentials uid=" + NFS_UID + ", gid=" + NFS_GID + ", username="
        + NFS_USER_NAME + ", groupname=" + NFS_GROUP_NAME);

    // Choose one endpoint to use (based on current node's hostname)
    String hostname = InetAddress.getLocalHost().getHostName();
    int chosenEndpoint = Math.abs((hostname == null ? 0 : hostname.hashCode())) % endpoints.size();
    LOG.info("Using endpoint " + chosenEndpoint + " with URI " + endpoints.get(chosenEndpoint) + " for NFS");
    
    // Initialize the filesystem store
    store = new NFSv3FileSystemStore(endpoints.get(chosenEndpoint), conf);
    store.initialize(endpoints.get(chosenEndpoint), conf);
    ...
    if (Rtmax > 0 && ((1 << readBlockSizeBits) > Rtmax)) {
      LOG.warn("Overwrite readBlockSize with Rtmax gotten from NFS server: "
    ...
    if (Wtmax > 0 && ((1 << writeBlockSizeBits) > Wtmax)) {
      LOG.warn("Overwrite writeBlockSize with Wtmax gotten from NFS server: "
  ...
  private int getNFSPortFromPortMap(URI uri) throws IOException {
    if (uri == null) {
      throw new IOException("URI is not defined yet!");
    }

    PortmapClient portmapClient = new PortmapClient(uri.getHost(), Nfs3Constant.SUN_RPCBIND);
    int nfsPort =
  ...
  public FSDataInputStream open(Path f, int bufferSize) throws IOException {
    ...
    return new FSDataInputStream(new BufferedFSInputStream(new NFSBufferedInputStream(
        configuration, store, handle, f, this.getConf(), this.getBlockSizeBits(), this.getUri()
            .getScheme(), this.getSplitSize(), statistics), bufferSize));
  ...
  public boolean delete(Path f, boolean recursive) throws IOException {
    ...
    if (fileType == NfsFileType.NFSREG.toValue()) {
      ...
      /*
       * Return false if recursive is not set. The upper layer seems to handle this case already.
      ...
      throw new IOException("Delete error: file type not supported beyond regular file and dir");
    }
  }
  ...
  private Boolean remove(Path file) throws IOException {
    ...
    REMOVE3Response remove3Response = store.remove(dirHandle, name, store.getCredentials());
  ...
  private Boolean rmdir(Path dir) throws IOException {
    ...
    RMDIR3Response rmdir3Response = store.rmdir(parentDirHandle, name, store.getCredentials());
    ...
    // Remove the stale handle from the handle cache
    if (handleCache.get(pathString) != null) {
      LOG.debug("remove handle " + handleCache.get(pathString) + " for file " + pathString);
      handleCache.remove(pathString);
    }
  ...
  private Set<Path> listSubPaths(Path path) throws IOException {
    ...
    while (true) {
      Nfs3DirList dirList =
          store.getDirectoryList(handle, cookie, cookieVerf, fsInfo.getDtpref(),
              store.getCredentials());
      if (dirList != null) {
  ...
  private boolean mkdir(FileHandle dir, String name, FsPermission permission) throws IOException {
    ...
    MKDIR3Response mkdir3Response = store.mkdir(dir, name, objAttr, store.getCredentials());
  ...
  private FileHandle create(FileHandle dir, String name, FsPermission permission)
    ...
    CREATE3Response create3Response =
        store.create(dir, name, Nfs3Constant.CREATE_UNCHECKED, objAttr, 0, store.getCredentials());
